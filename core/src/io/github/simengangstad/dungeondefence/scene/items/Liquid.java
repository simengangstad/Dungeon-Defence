...
import io.github.simengangstad.dungeondefence.scene.Collidable;
...
public class Liquid extends Collidable {
    ...
    public void draw(SpriteBatch batch) {
        ...
        batch.draw((TextureRegion) animationContent.getKeyFrame(stateTime), position.x - size.x / 2.0f, position.y - size.y / 2.0f, size.x, size.y);

        batch.setColor(Color.WHITE);

        batch.draw((TextureRegion) animationFrame.getKeyFrame(stateTime), position.x - size.x / 2.0f, position.y - size.y / 2.0f, size.x, size.y);
    }
